import { state, escapeHtml, updateTranscriptFilter, formatTime } from './utils.js';
...
export function renderStats(steps) {
    ...
    const sortedSteps = [...steps].filter(s => s.created_at).sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime());
    ...
    steps.forEach(step => {
        ...
        if (step.type && (step.type.includes('TOOL') || step.type.includes('VIEW_FILE') || step.type.includes('COMMAND'))) {
            toolsCalled++;
        }
        ...
        if (step.source === 'MODEL' && (step.type === 'PLANNER_RESPONSE' || step.type === 'MESSAGE') && step.content && (!step.tool_calls || step.tool_calls.length === 0)) {
            modelResponses++;
        }
        ...
        if (isErrorStep) {
            ...
            if (step.type === 'RUN_COMMAND') {
                ...
                let lines = source.split('\n').map(l => l.trim()).filter(l => l && !l.startsWith('Created At:') && !l.startsWith('Completed At:'));
                if (lines.length > 0) {
                    errMsg = lines[0];
    ...
    let durationChartHtml = '<div class="tools-chart" id="duration-chart" style="display: none; margin-top: 24px; padding-top: 16px; border-top: 1px solid var(--border-color);">';
    durationChartHtml += '<div style="font-size: 0.75rem; font-weight: 700; color: #f59e0b; margin-bottom: 16px; letter-spacing: 0.05em; text-transform: uppercase;">Session Timeline</div>';
    ...
    if (segments.length === 0) {
        ...
        const defaultSummary = `<span>Active: <strong style="color:var(--text-primary);">${formatMs(totalActiveMs) === '0s' ? 'Instant' : formatMs(totalActiveMs)}</strong></span><span>Paused: <strong style="color:var(--text-primary);">${formatMs(totalPausedMs)}</strong></span>`;

        durationChartHtml += '<div style="display: flex; height: 24px; width: 100%; background: rgba(30, 41, 59, 0.4); border-radius: 6px; overflow: hidden; border: 1px solid rgba(255,255,255,0.05);">';
    ...
    const sortedTools = Object.entries(toolFrequencies).sort((a, b) => b[1] - a[1]);
    let toolsChartHtml = '<div class="tools-chart" id="tools-chart" style="display: none; margin-top: 24px; padding-top: 16px; border-top: 1px solid var(--border-color);">';
    toolsChartHtml += '<div style="font-size: 0.75rem; font-weight: 700; color: var(--text-secondary); margin-bottom: 16px; letter-spacing: 0.05em; text-transform: uppercase;">Tool Distribution</div>';
    ...
    const sortedErrors = Object.entries(errorFrequencies).sort((a, b) => b[1] - a[1]);
    let errorsChartHtml = '<div class="tools-chart" id="errors-chart" style="display: none; margin-top: 24px; padding-top: 16px; border-top: 1px solid var(--border-color);">';
    errorsChartHtml += '<div style="font-size: 0.75rem; font-weight: 700; color: var(--error); margin-bottom: 16px; letter-spacing: 0.05em; text-transform: uppercase;">Issues Breakdown</div>';
    ...
    if (sortedErrors.length === 0) {
        ...
        sortedErrors.forEach(([name, count]) => {
            ...
            errorsChartHtml += `
                <div class="tool-chart-row" style="display: flex; align-items: center; margin-bottom: 10px; gap: 16px;">
                    <div style="flex: 0 0 300px; font-size: 0.8rem; font-family: var(--font-mono); color: var(--text-primary); text-align: right; text-overflow: ellipsis; overflow: hidden; white-space: nowrap;" title="${escapeHtml(name)}">${escapeHtml(name)}</div>
                    <div style="flex: 1; height: 8px; background: rgba(30, 41, 59, 0.5); border-radius: 4px; overflow: hidden;">
    ...
    container.innerHTML = `
        <div class="stats-grid">
            <div class="stat-card" id="errors-stat-card" style="cursor: ${errors > 0 ? 'pointer' : 'default'};">
                <div class="stat-label" style="display:flex; justify-content:space-between; align-items:center;">
                    OUTCOME
                    <span id="errors-chevron" class="chevron" style="display: ${errors > 0 ? 'inline-block' : 'none'};">›</span>
                </div>
                <div class="stat-value" style="color: ${errors === 0 ? 'var(--success)' : 'var(--error)'};">
            ...
            <div class="stat-card" id="duration-stat-card" style="cursor: pointer;">
                ...
                <div class="stat-sub">${segments.length} active segment${segments.length !== 1 ? 's' : ''}</div>
            </div>
