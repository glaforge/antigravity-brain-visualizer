...
export function renderTranscript(steps, container) {
    state.activeFilters = { userQueries: false, toolsCalled: false, outcomeErrors: false, modelResponses: false };
    container.innerHTML = '';
    
    if (!steps || steps.length === 0) {
        container.innerHTML = '<div class="empty-state">No transcript data found.</div>';
        return;
    }
    
    steps.forEach((step, index) => {
        const isUserStep = step.source === 'USER_EXPLICIT' || step.type === 'USER_INPUT';
        const isErrorStep = step.status === 'ERROR' || step.type === 'ERROR_MESSAGE' || (step.type === 'RUN_COMMAND' && step.content && step.content.includes('The command failed'));
        const isFinalModelResponse = step.source === 'MODEL' && (step.type === 'PLANNER_RESPONSE' || step.type === 'MESSAGE') && step.content && (!step.tool_calls || step.tool_calls.length === 0);
        ...
        if (step.source === 'USER_EXPLICIT') {
            ...
            if (step.type && step.type !== 'PLANNER_RESPONSE' && step.type !== 'MESSAGE' && step.type !== 'ASK_QUESTION') {
                badgeClass = 'tool';
            } else {
                badgeClass = 'model';
            }
        } else if (step.type && (step.type.includes('TOOL') || step.type.includes('VIEW_FILE') || step.type.includes('COMMAND'))) {
            badgeClass = 'tool';
        }
        
        card.dataset.isUser = isUserStep ? 'true' : 'false';
        card.dataset.isTool = (badgeClass === 'tool' || (step.tool_calls && step.tool_calls.length > 0)) ? 'true' : 'false';
        card.dataset.isError = isErrorStep ? 'true' : 'false';
        card.dataset.isModel = isFinalModelResponse ? 'true' : 'false';
        
        const hasContent = step.content || step.thinking || step.error || (step.tool_calls && step.tool_calls.length > 0);
        ...
        header.innerHTML = `
            <div style="display:flex; align-items:center; gap:12px;">
                ${hasContent ? '<span class="chevron">›</span>' : '<span style="width:16px;"></span>'}
            ...
            <div class="step-meta ${badgeClass}">${step.created_at ? `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><polyline points="12 6 12 12 16 14"></polyline></svg> ` + formatTime(step.created_at, true) : ''}</div>
        `;
        ...
        if (step.content) {
            ...
            if (step.type === 'USER_INPUT' || step.type === 'PLANNER_RESPONSE' || step.type === 'MESSAGE' || step.type === 'SEARCH_WEB') {
                
                let processedContent = step.content;
                if (step.type === 'USER_INPUT') {
                    let fileMap = {};
                    const metadataMatch = processedContent.match(/<ADDITIONAL_METADATA>([\s\S]*?)<\/ADDITIONAL_METADATA>/);
                    if (metadataMatch) {
                ...
                if (step.type === 'USER_INPUT' && processedContent.includes('<')) {
                    ...
                    while ((match = tagRegex.exec(processedContent)) !== null) {
                        ...
                        if (match.index > lastIndex) {
                            const preText = processedContent.substring(lastIndex, match.index).trim();
                            if (preText) htmlParts += `<div class="user-request-block">${marked.parse(preText)}</div>`;
                        }
                        const tagName = match[1];
                        const tagContent = match[2].trim();
                        if (tagName === 'USER_REQUEST') {
                            htmlParts += `<div class="user-request-block">${marked.parse(tagContent)}</div>`;
                        } else {
                            const niceName = tagName.split('_').map(w => w.charAt(0) + w.slice(1).toLowerCase()).join(' ');
                            htmlParts += `<div class="system-context-block"><strong>${niceName}</strong><div class="system-context-content">${marked.parse(tagContent)}</div></div>`;
                        }
                        lastIndex = tagRegex.lastIndex;
                    }
                    if (lastIndex < processedContent.length) {
                        const postText = processedContent.substring(lastIndex).trim();
                        if (postText) htmlParts += `<div class="user-request-block">${marked.parse(postText)}</div>`;
                    }
                    formattedContent = `<div class="markdown-body">${hasTags ? htmlParts : marked.parse(processedContent)}</div>`;
                } else {
                    ...
                    if (step.type === 'SEARCH_WEB') {
                        ...
                        if (match && match[0]) {
                            ...
                            const searchPhraseRegex = /^The search for "(.*?)" returned the following summary:\n/i;
                            const searchMatch = contentText.match(searchPhraseRegex);
                            let searchPhrase = '';
                            if (searchMatch) {
                                searchPhrase = searchMatch[1].trim();
                                contentText = contentText.substring(searchMatch[0].length).trim();
                            }
                            ...
                            if (created && completed) {
                                metaHtml += `<div>⏱ <strong>Duration:</strong> ${((new Date(completed) - new Date(created))/1000).toFixed(1)}s</div>`;
                            } else if (created) {
                                metaHtml += `<div>⏱ <strong>Started:</strong> ${formatTime(created, true)}</div>`;
                            }
                            if (searchPhrase) {
                                metaHtml += `<div>🔍 <strong>Query:</strong> ${escapeHtml(searchPhrase)}</div>`;
                            }
                            if (errorMsg) {
                                metaHtml += `<div style="color:#ef4444;">🚨 <strong>Error:</strong> ${escapeHtml(errorMsg.trim())}</div>`;
                            }
                            metaHtml += `</div>`;
                            prefixHtml = metaHtml;
                        }
                        ...
                        let linkMap = {};
                        const defRegex = /\[(\d+)\]\s*(?:\[.*?\]\((https?:\/\/[^\s\)]+)\)|(https?:\/\/[^\s\)]+))/g;
                        let m;
                        ...
                        contentText = contentText.replace(seqRegex, (match, offset, str) => {
                            const after = str.substring(offset + match.length);
                            if (after.match(/^\s*\[.*?\]\(http/) || after.match(/^\s*http/)) {
                            ...
                            let linksHtml = ids.map(id => {
                                if (linkMap[id]) return `<a href="${linkMap[id]}" target="_blank" style="text-decoration:none; color:var(--accent-blue); font-weight:600;">${id}</a>`;
                                return id;
                            }).join(', ');
                        ...
                        contentText = contentText.replace(/\[(\d+)\]\s*(?:\[.*?\]\((https?:\/\/[^\s\)]+)\)|(https?:\/\/[^\s\)]+))/g, '\n\n- $&');
                    }
                    formattedContent = prefixHtml + `<div class="markdown-body">${marked.parse(contentText)}</div>`;
                ...
                if (match && match[0]) {
                    ...
                    let statusMsg = '';
                    const statusRegex = /^\s*(The command (completed successfully\.|failed with exit code: \d+))/m;
                    ...
                    if (created || completed || errorMsg || statusMsg) {
                        ...
                        if (statusMsg) {
                            ...
                            metaHtml += `<div style="color:${color};">${icon} <strong>Status:</strong> ${escapeHtml(statusMsg)}</div>`;
                        }
                        if (errorMsg) {
                            metaHtml += `<div style="color:#ef4444;">🚨 <strong>Error:</strong> ${escapeHtml(errorMsg.trim())}</div>`;
                        }
                ...
                contentText = contentText.replace(/^(?:Output|Stdout|Stderr):\s*\n?/gm, '').trim();
                ...
                if (!contentText) {
                    ...
                    try {
                        ...
                        if (typeof parsed === 'object' && parsed !== null) {
                            formattedContent = syntaxHighlight(JSON.stringify(parsed, null, 2));
                        }
                    } catch(e) {}
                    formattedContent = metaHtml + `<div class="code-block">${formattedContent}</div>`;
                }
            }
            ...
            step.tool_calls.forEach(tool => {
                html += `
                    <div class="tool-call">
                        ...
                        <pre class="tool-args json-renderer">${syntaxHighlight(JSON.stringify(tool.args, null, 2))}</pre>
                    </div>
                `;
            });
        }
